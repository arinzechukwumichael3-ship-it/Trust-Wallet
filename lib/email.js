 ...
 *
 * Deliverability notes (why emails can land in spam and how this avoids it):
 *   - The From name is just "Support" and the template never mentions an
 *     unrelated brand, so the content matches the sending domain (yieldempire.org).
 *   - We send both an HTML and a plain-text part (multi-part) — single-part HTML
 *     is a common spam signal.
 *   - A List-Unsubscribe header is included so mailbox providers treat this as a
 *     legitimate transactional/notification message.
...
// Plain-text rendering of the message body (used for the multipart text part).
...
// Shared visual shell. Brand-neutral: only "Support" / "Helpry", matching the
// sending domain so mailbox filters don't flag a brand/domain mismatch.
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(5,0,255,0.08);">
        ...
        <tr><td style="background:linear-gradient(135deg,#0500FF,#00d395);padding:24px 32px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="font-size:18px;font-weight:700;color:#ffffff;letter-spacing:.2px;">Support</td>
              <td align="right" style="font-size:12px;color:rgba(255,255,255,0.85);">Helpry</td>
            </tr>
          </table>
        </td></tr>
        <!-- Body -->
        ...
        <tr><td style="padding:20px 32px;background-color:#f8fafc;border-top:1px solid #e2e8f0;">
          <p style="margin:0;font-size:12px;line-height:1.6;color:#64748b;">
            This is an automated message from Support. You are receiving this because you are part of a support conversation.
          ...
          <p style="margin:12px 0 0;font-size:11px;color:#94a3b8;">© ${year} Support · Helpry</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
// so providers treat this as a legitimate notification (lowers spam scoring).
...
async function notifySupportOfClientMessage(c, m) {
  ...
  return sendEmail({
    ...
    bodyInner: `
      ...
      <p style="margin:0 0 20px;font-size:14px;color:#64748b;">A customer just messaged you through the Helpry support chat.</p>
      ...
      <p style="margin:20px 0 0;">
        <a href="${adminUrl}" style="display:inline-block;background-color:#0500FF;color:#ffffff;font-size:14px;font-weight:600;padding:12px 22px;border-radius:10px;text-decoration:none;">Open admin console</a>
      </p>
    `,
    bodyLines: [
      'A customer just messaged you through the Helpry support chat.',
      '',
...
async function notifyClientOfReply(c, m) {
  ...
  const chatUrl = `${APP_URL}/helpry.jp/cmupnn-trustwallet-xostfj-helpry-eohlok-trustwallet-gkqtyx.html`;
  return sendEmail({
    to: [c.email],
    subject,
    title: subject,
    ...
    bodyInner: `
      ...
      <p style="margin:20px 0 0;">
        <a href="${chatUrl}" style="display:inline-block;background-color:#0500FF;color:#ffffff;font-size:14px;font-weight:600;padding:12px 22px;border-radius:10px;text-decoration:none;">Open the chat</a>
      </p>
