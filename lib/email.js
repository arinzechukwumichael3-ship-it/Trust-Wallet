...
// Common send wrapper: HTML + plain-text multipart, plus a compliant
// RFC 8058 one-click HTTPS List-Unsubscribe header (Gmail bulk-sender rule).
async function sendEmail({ to, subject, title, preheader, bodyInner, bodyLines, replyTo, unsubEmail }) {
  const cl = getClient();
  ...
  const unsubAddr = encodeURIComponent(unsubEmail || SUPPORT_EMAIL);
  const unsubUrl = `${APP_URL}/api/unsubscribe?email=${unsubAddr}`;
  try {
    const { data, error } = await cl.emails.send({
      ...
      text,
      headers: {
        'List-Unsubscribe': `<${unsubUrl}>, <mailto:${SUPPORT_EMAIL}?subject=unsubscribe>`,
